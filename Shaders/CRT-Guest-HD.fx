...
float4 ChromaticPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	if (edgemask > 0.05) {
		mx0 = COMPAT_TEXTURE(CRTHD_S7, pos1 - dx).a; mx0 = COMPAT_TEXTURE(CRTHD_S7, pos1 - dx*(1.0-0.75*sqrt(mx0))).a; 
		mx2 = COMPAT_TEXTURE(CRTHD_S7, pos1 + dx).a; mx2 = COMPAT_TEXTURE(CRTHD_S7, pos1 + dx*(1.0-0.75*sqrt(mx2))).a; 
		float mx3 = COMPAT_TEXTURE(CRTHD_S7, pos1 - 4.0*dx).a;
		float mx4 = COMPAT_TEXTURE(CRTHD_S7, pos1 + 4.0*dx).a;
