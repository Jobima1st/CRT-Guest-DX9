...
float4 SharpnessPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	float l01=get_lumad(tex2D(NTSC_S01,texcoord+2.0*g01).rgb);
	float l21=get_lumad(tex2D(NTSC_S01,texcoord+2.0*g21).rgb);
	float l11=get_lumad(tex2D(NTSC_S01,tex0            ).rgb);
