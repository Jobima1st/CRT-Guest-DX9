...
void bring_pixel(inout float3 c,inout float3 b,inout float3 g,float2 coord,float2 boord)
{
	...
	float r1=COMPAT_TEXTURE(GUEST_S6,coord+rc).r;
	float g1=COMPAT_TEXTURE(GUEST_S6,coord+gc).g;
	float b1=COMPAT_TEXTURE(GUEST_S6,coord+bc).b;
	float ds=decons;
...
{
	float3 c1=COMPAT_TEXTURE(GUEST_S0,fuxcoord).rgb;
	float3 c2=COMPAT_TEXTURE(GUEST_S0,fuxcoord+float2(0.0,OrgSize.w)).rgb;
...
float4 ChromaticPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	if (edgemask > 0.05) {
		...
		mx2 = COMPAT_TEXTURE(GUEST_S7, pos1 + dx).a; mx2 = COMPAT_TEXTURE(GUEST_S7, pos1 + dx*(1.0-0.75*sqrt(mx2))).a; 
		float mx3 = COMPAT_TEXTURE(GUEST_S7, pos1 - 4.0*dx).a;
		float mx4 = COMPAT_TEXTURE(GUEST_S7, pos1 + 4.0*dx).a;
